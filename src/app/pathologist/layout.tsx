...
export default async function PathologistLayout({ children }: { children: React.ReactNode }) {
  ...
  const [profileRes, unreadRes] = await Promise.all([
    ...
    supabase.from('notifications').select('id', { count: 'exact', head: true }).eq('read', false).eq('role', 'pathologist'),
  ]);
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--d-page)' }}>
      ...
      <MobileSidebarDrawer navItems={navWithBadge} profile={profile} roleSwitcher={<RoleSwitcher currentRole="pathologist" allRoles={roles} />} />
