...
export default async function TransporterNotificationsPage() {
  ...
  if (unread > 0) {
    await supabase.from('notifications').update({ read: true }).eq('user_id', user.id).eq('read', false).or('role.eq.transporter,role.is.null');
  }
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      {notifications.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 24px', background: C.cardBg, borderRadius: 16, boxShadow: C.cardShadow }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 700, color: C.text }}>No notifications yet</p>
          <p style={{ fontSize: 13, color: C.muted, marginTop: 4 }}>Job requests and payment alerts will appear here.</p>
        </div>
      ) : (
            ...
            return (
              <div key={n.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {n.body && <p style={{ fontSize: 12, color: C.muted, margin: '0 0 4px', lineHeight: 1.4 }}>{n.body}</p>}
