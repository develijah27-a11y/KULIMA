...
const STATUS_CFG: Record<string, { color: string; bg: string; label: string; icon: string }> = {
  open:       { color: C.green,  bg: 'var(--color-primary-bg)', label: 'Open',        icon: '🔓' },
  assigned:   { color: C.blue,   bg: 'var(--color-sky-bg)',     label: 'Assigned',    icon: '🚗' },
  in_transit: { color: C.amber,  bg: 'var(--color-harvest-bg)', label: 'In Transit',  icon: '🚛' },
  delivered:  { color: C.purple, bg: '#EDE9FE',                 label: 'Delivered',   icon: '✅' },
  cancelled:  { color: C.red,    bg: 'var(--color-danger-bg)',  label: 'Cancelled',   icon: '✕' },
};

const TYPE_ICON: Record<string, string> = { standard: '🚛', fast: '⚡', cold: '❄️' };
...
async function TransporterStats({ userId }: { userId: string }) {
  ...
  const stats = [
    { label: 'Active Jobs',      value: activeRes.count ?? 0,                                                    sub: 'Assigned & in transit',  icon: '🚛', border: C.green },
    { label: 'New Requests',     value: pendingRes.count ?? 0,                                                   sub: 'Awaiting your response', icon: '📬', border: C.amber },
    { label: 'Earned This Week', value: `UGX ${Math.round(weekEarnings).toLocaleString()}`,                      sub: 'Last 7 days — paid jobs', icon: '💰', border: C.blue },
    { label: 'Wallet Balance',   value: `UGX ${Math.round(balance).toLocaleString()}`,                           sub: 'Available to withdraw',  icon: '💳', border: C.purple },
  ];
...
function QuickActions() {
  const actions = [
    { label: 'Job Queue',  href: '/transporter/job-queue',  emoji: '📬', bg: 'var(--color-primary-bg)', color: C.green },
    { label: 'Active Jobs',href: '/transporter/active',     emoji: '🚛', bg: 'var(--color-sky-bg)',     color: C.blue },
    { label: 'Earnings',   href: '/transporter/wallet',     emoji: '💰', bg: 'var(--color-harvest-bg)', color: C.amber },
    { label: 'My Vehicle', href: '/transporter/vehicle',    emoji: '🚗', bg: '#EDE9FE',                 color: C.purple },
    { label: 'Cold Chain', href: '/transporter/cold-chain', emoji: '❄️', bg: 'var(--color-danger-bg)',  color: C.red },
  ];
  ...
  return (
    <Card style={{ padding: '16px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ width: 52, height: 52, borderRadius: 14, background: 'var(--color-primary-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, flexShrink: 0 }}>
          🚛
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            ...
            {vehicle.is_cold_capable && (
              <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 999, background: 'var(--color-sky-bg)', color: C.blue }}>❄️ Cold Chain</span>
            )}
          </div>
...
async function OpenJobs({ userId }: { userId: string }) {
  ...
  // Pending assignments (system matched)
      ...
      {pendingRows.length > 0 ? (
        <div className="divide-y" style={{ borderColor: C.border }}>
          {pendingRows.map((a: any) => {
            const d = a.delivery;
            if (!d) return null;
            const typeIcon = TYPE_ICON[d.delivery_type ?? 'standard'] ?? '🚛';
            return (
        ...
        <div className="divide-y" style={{ borderColor: C.border }}>
          {openRows.slice(0, 5).map((job: any) => {
            const typeIcon = TYPE_ICON[job.delivery_type ?? 'standard'] ?? '🚛';
            return (
          ...
          <div key={d.id} className="px-5 py-3.5 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center text-base shrink-0"
                style={{ background: d.payment_status === 'paid' ? 'var(--color-success-bg)' : 'var(--color-surface-2)' }}>
                {TYPE_ICON[d.delivery_type ?? 'standard'] ?? '🚛'}
              </div>
              <div>
            ...
            <div className="text-right shrink-0">
              ...
              <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full"
                style={{
                  background: d.payment_status === 'paid' ? 'var(--color-success-bg)' : 'var(--color-harvest-bg)',
                  color: d.payment_status === 'paid' ? 'var(--color-success)' : C.amber,
                }}>
