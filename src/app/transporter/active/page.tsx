  ...
  const [{ data: pending }, { data: active }, { data: completed }] = await Promise.all([
    (supabase.from as any)('driver_assignments')
      .select(`
        ...
        delivery:delivery_requests(
          ...
          requester:profiles!delivery_requests_requester_profile_fkey(full_name, phone_number, location)
        )
      `)
    ...
    (supabase.from as any)('delivery_requests')
      .select(`
        ...
        distance_km, status, trip_phase, accepted_at, picked_up_at, pickup_date,
        requester:profiles!delivery_requests_requester_profile_fkey(full_name, phone_number, location)
      `)
