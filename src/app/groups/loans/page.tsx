...
import { ClipboardList, CheckCircle2, CalendarDays, BarChart3, Users } from 'lucide-react';
...
const FEATURES: { icon: JSX.Element; title: string; desc: string }[] = [
  { icon: <ClipboardList size={18} />, title: 'Loan Applications',  desc: 'Members apply for short-term loans from the group pool with digital forms.' },
  { icon: <CheckCircle2 size={18} />,  title: 'Committee Approval', desc: 'Group leaders vote to approve or reject applications via the app.' },
  { icon: <CalendarDays size={18} />,  title: 'Repayment Schedules',desc: 'Automatic repayment tracking with SMS reminders before due dates.' },
  { icon: <BarChart3 size={18} />,     title: 'Credit Scoring',     desc: 'Build member credit history based on repayment behaviour.' },
];
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, textAlign: 'left', maxWidth: 400, margin: '0 auto' }}>
            {FEATURES.map(f => (
              <div key={f.title} style={{ display: 'flex', gap: 12, padding: '12px 16px', borderRadius: 12, background: 'var(--d-bg)' }}>
                <span style={{ display: 'flex', flexShrink: 0, color: C.blue }}>{f.icon}</span>
                <div>
