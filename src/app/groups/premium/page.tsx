...
import { Check, Zap, ArrowRight, Shield, Users } from 'lucide-react';
...
const C = {
  ...
  green: 'var(--color-primary)', gold: 'var(--color-harvest)',
} as const;

const GROUP_PRO_FEATURES = [
  'Bulk listing tools for the whole group',
  'Group-level sales & performance analytics',
  'Priority matching to bulk buyers',
  'Group wallet & member disbursement tracking',
  'Discounted per-member seats as you grow',
  'Training content for group leaders',
];

export default async function GroupsPremiumPage() {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('id, full_name, role, roles, subscription_tier, role_subscription_tiers')
    .eq('user_id', user.id)
    .single();

  const { data: group } = profile
    ? await (supabase.from as any)('farmer_groups').select('id, name, wallet_balance, leader_id').eq('leader_id', (profile as any).id).maybeSingle()
    : { data: null };

  const isLeader = !!group;
  const currentTier = getEffectiveTier(profile as any, 'groups');
  const isAlreadyPro = currentTier !== 'free';
  const plan = PLANS.group_pro;

  const { data: activeSub } = isAlreadyPro
    ? await (supabase.from as any)('subscriptions')
        .select('id').eq('role', 'groups').eq('status', 'active').maybeSingle()
    : { data: null };
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p style={{ fontSize: 13, color: C.muted, margin: '6px 0 0' }}>
          Sell as one large lot with tools built for cooperatives — billed from your group's wallet, not your personal one.
        </p>
      </div>

      {!isLeader && (
        <div style={{ padding: '14px 18px', borderRadius: 14, background: 'var(--color-harvest-bg)', border: '1px solid var(--color-harvest)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <Users size={20} style={{ color: 'var(--color-harvest)', flexShrink: 0 }} />
          <p style={{ fontSize: 13.5, fontWeight: 700, color: C.text, margin: 0 }}>
            Only the group's leader can purchase Group Pro — ask them to visit this page.
          </p>
        </div>
      )}
        ...
        <div style={{ padding: '14px 18px', borderRadius: 14, background: 'var(--color-success-bg)', border: '1px solid var(--color-success-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <p style={{ fontSize: 14, fontWeight: 700, color: 'var(--color-success)', margin: 0 }}>
              {group?.name ?? 'Your group'} is already on the {currentTier.replace(/_/g, ' ')} plan. Enjoy!
            </p>
          </div>
      ...
      <div style={{ background: C.gold, borderRadius: 20, padding: '28px 24px', boxShadow: `0 16px 40px ${C.gold}30, ${C.shadow}`, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.7)', margin: '2px 0 0' }}>Flat fee per group, not per member</p>
          </div>
        </div>
        ...
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.75)', margin: '0 0 20px' }}>
          ...
          {isLeader && group ? ` · billed from ${group.name}'s wallet (UGX ${Number(group.wallet_balance ?? 0).toLocaleString()} available)` : ''}
        </p>

        <div style={{ height: 1, background: 'rgba(255,255,255,0.2)', marginBottom: 20 }} />

        <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 24px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {GROUP_PRO_FEATURES.map(f => (
        ...
        {!isAlreadyPro && isLeader && (
          <SubscribeButton
            ...
            style={{
              ...
              background: '#fff', color: C.gold, fontWeight: 900, fontSize: 15,
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        {['Cancel any time — no lock-in', "Paid via your group's wallet", '100% secure — no card needed'].map(t => (
