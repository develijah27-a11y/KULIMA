...
const BUYER_PRO_FEATURES = [
  ...
  'Saved suppliers & favourite farmers',
  'Priority matching on group listings',
  'Bulk sourcing requests',
  'Auto-generated invoices for every order',
  'Priority support — response within 4 hours',
];

export default async function BuyerPremiumPage() {
  ...
  const currentTier = getEffectiveTier(profile as any, 'buyer');
  const isAlreadyPro = currentTier !== 'free';
  const plan = PLANS.buyer_pro;

  const { data: activeSub } = isAlreadyPro
    ? await (supabase.from as any)('subscriptions')
        .select('id').eq('role', 'buyer').eq('status', 'active').maybeSingle()
    : { data: null };
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h1 style={{ fontSize: 22, fontWeight: 900, color: C.text, letterSpacing: '-0.03em', margin: 0, fontFamily: "'Poppins','Inter',system-ui,sans-serif" }}>
          Upgrade to Buyer Pro
        </h1>
        <p style={{ fontSize: 13, color: C.muted, margin: '6px 0 0' }}>
          Source with confidence — alerts, bulk sourcing, and faster order matching.
        </p>
      </div>
      ...
      <div style={{ background: C.sky, borderRadius: 20, padding: '28px 24px', boxShadow: `0 16px 40px ${C.sky}30, ${C.shadow}`, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.7)', margin: '2px 0 0' }}>For active produce buyers</p>
          </div>
        </div>
