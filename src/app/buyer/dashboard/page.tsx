...
function QuickActions() {
  const actions = [
    ...
    { label: 'Request Driver', href: '/buyer/deliveries/new', icon: <Zap size={20} />, bg: 'var(--color-harvest-bg)', color: C.amber },
...
async function WelcomeHeader({ userId }: { userId: string }) {
  ...
  return (
    <DashboardWelcomeHero
      ...
      actionIcon={<ShoppingBag size={15} />}
      secondaryAction={{
        href: '/buyer/orders',
        label: 'My Orders',
        icon: <Package size={14} className="text-sky-500" />,
      }}
    />
  );
}
...
export default async function BuyerDashboardPage() {
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      ...
      <Suspense fallback={<DashboardWelcomeHeroSkeleton />}>
    ...
    <Card>
      <div className="px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3" style={{ borderBottom: `1px solid ${C.border}` }}>
        <div>
          ...
          <p className="text-xs" style={{ color: C.muted, margin: '2px 0 0' }}>Need transport for your goods? Request a verified driver in seconds.</p>
        </div>
        <Link
