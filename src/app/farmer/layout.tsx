...
export default async function FarmerLayout({ children }: { children: React.ReactNode }) {
  ...
  const [profileRes, unreadRes, deliveryRes] = await Promise.all([
    ...
    supabase.from('notifications').select('id', { count: 'exact', head: true }).eq('read', false).eq('role', 'farmer'),
    (supabase.from as any)('delivery_requests').select('id', { count: 'exact', head: true }).eq('requester_id', user.id).in('status', ['open', 'assigned', 'in_transit']).eq('requester_role', 'farmer'),
  ]);
