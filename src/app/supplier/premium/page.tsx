...
import { Check, Building2, Crown, ArrowRight, Shield } from 'lucide-react';
...
const C = {
  ...
  green: 'var(--color-primary)', sky: 'var(--color-sky)',
} as const;
...
export default async function SupplierPremiumPage() {
  ...
  const currentTier = getEffectiveTier(profile as any, 'supplier');
  const isAlreadyPro = currentTier === 'business' || currentTier === 'enterprise';
  const plan = PLANS.business;

  const { data: activeSub } = isAlreadyPro
    ? await (supabase.from as any)('subscriptions')
        .select('id').eq('role', 'supplier').eq('status', 'active').maybeSingle()
    : { data: null };
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p style={{ fontSize: 13, color: C.muted, margin: '6px 0 0' }}>
          Staff accounts, inventory tools, and analytics for growing agro-dealers.
        </p>
      </div>
        ...
        <div style={{ padding: '14px 18px', borderRadius: 14, background: 'var(--color-success-bg)', border: '1px solid var(--color-success-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <p style={{ fontSize: 14, fontWeight: 700, color: 'var(--color-success)', margin: 0 }}>
              You are already on the {currentTier} plan. Enjoy!
            </p>
          </div>
      ...
      <div style={{ background: C.sky, borderRadius: 20, padding: '28px 24px', boxShadow: `0 16px 40px ${C.sky}30, ${C.shadow}`, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.7)', margin: '2px 0 0' }}>For agro-dealers & growing operations</p>
          </div>
        </div>
        ...
        {!isAlreadyPro && (
          <SubscribeButton
            ...
            style={{
              ...
              background: '#fff', color: C.sky, fontWeight: 900, fontSize: 15,
      ...
      {/* Enterprise — not self-serve, matches src/lib/subscriptions/plans.ts selfServe:false */}
      <div style={{ background: C.card, borderRadius: 16, boxShadow: C.shadow, padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 14, border: `1px solid ${C.border}` }}>
        <div style={{ width: 40, height: 40, borderRadius: 11, background: 'var(--color-purple-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <Crown size={20} style={{ color: 'var(--color-purple)' }} />
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: C.text, margin: 0 }}>Need multi-branch or API access?</p>
          <p style={{ fontSize: 12, color: C.muted, margin: '3px 0 0' }}>Enterprise is a custom plan for large distributors — talk to us.</p>
        </div>
        <Link href="/contact" style={{ padding: '9px 18px', borderRadius: 10, background: 'var(--color-purple)', color: '#fff', fontWeight: 800, fontSize: 13, flexShrink: 0, whiteSpace: 'nowrap', textDecoration: 'none' }}>
          Contact us
        </Link>
      </div>
