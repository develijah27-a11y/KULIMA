...
export async function GET(request: NextRequest) {
  try {
    ...
    const { farmId, page, limit, startDate, endDate } = validation.data;
    const { weatherLogs, total } = await getWeatherLogsByFarm(
      farmId,
      user.id,
      page,
      limit,
    ...
    return NextResponse.json({
      success: true,
      data: buildPaginatedResponse(weatherLogs, page, limit, total),
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const weatherLog = await createWeatherLog(validation.data, user.id);
    return NextResponse.json({ success: true, data: { weatherLog } }, { status: 201 });
  } catch (error) {
    const { response, statusCode } = handleError(error);
    return NextResponse.json(response, { status: statusCode });
  }
