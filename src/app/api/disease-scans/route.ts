...
export async function GET(request: NextRequest) {
  try {
    ...
    const { diseaseScans, total } = await getDiseaseScansByFarm(farmId, user.id, page, limit);

    return NextResponse.json({
      success: true,
      data: buildPaginatedResponse(diseaseScans, page, limit, total),
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const diseaseScan = await createDiseaseScan(validation.data, user.id);
    return NextResponse.json({ success: true, data: { diseaseScan } }, { status: 201 });
  } catch (error) {
    const { response, statusCode } = handleError(error);
    return NextResponse.json(response, { status: statusCode });
  }
