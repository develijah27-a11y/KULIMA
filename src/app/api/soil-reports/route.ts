...
export async function GET(request: NextRequest) {
  try {
    ...
    const { soilReports, total } = await getSoilReportsByFarm(farmId, user.id, page, limit);

    return NextResponse.json({
      success: true,
      data: buildPaginatedResponse(soilReports, page, limit, total),
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const soilReport = await createSoilReport(validation.data, user.id);
    return NextResponse.json({ success: true, data: { soilReport } }, { status: 201 });
  } catch (error) {
    const { response, statusCode } = handleError(error);
    return NextResponse.json(response, { status: statusCode });
  }
