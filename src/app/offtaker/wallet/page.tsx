...
export default async function OfftakerWalletPage() {
  ...
  const holderName = (profileRes.data as any)?.full_name ?? 'Cropify Offtaker';
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <div style={{ background: C.cardBg, borderRadius: 14, boxShadow: C.cardShadow, padding: '14px 16px' }}>
          <p style={{ fontSize: 10, fontWeight: 700, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 4px' }}>Total Spent</p>
          <p style={{ fontSize: 16, fontWeight: 800, color: C.text, margin: 0, fontFamily: 'var(--font-mono)' }}>UGX {Math.round(totalSpent).toLocaleString()}</p>
        </div>
      </div>
